...
| 禁止 | 单次会话实现「整个 V1」 |
| 验证 | 有 UI：**`-web` 阶段** 须 `pnpm build` + `pnpm typecheck` + 浏览器路径；integrate 须联调 |
...
| 未登录 | `/app/**`（除 login）、`/admin/**` 强制跳转 login；禁止未登录渲染壳层 |
| 登录后 | `/app/messages` 工作台（有效组织成员即可，不按 RBAC 分入口） |
| 管理后台 | `/admin/*`；同一 JWT；须 **管理身份**；内部 RBAC 控菜单/API |
