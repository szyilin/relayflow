...
| 基础 Mapper | `SysUserMapper.java` | `src/main/java/.../dal/mysql/` | 同上（`BaseMapper<T>`） |
| 基础 XML | `SysUserMapper.xml` | `src/main/resources/mapper/` | 同上（resultMap / 列清单 / 基础 CRUD） |
| 自定义接口 | `SysUserExtMapper.java` | `src/main/java/.../dal/mysql/` | **手写**，进 Git |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `--module` / `-m` | 是 | `system` / `infra` / `im` / `task`（见 `codegen.yml`） |
