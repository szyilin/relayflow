...
RelayFlow 面向企业协作：**同一账号**登录后，根据是否具备**管理角色**，分别使用**员工工作台（产品面）**或**管理后台（管理面）**。本文档是产品权限的**开发约定真源**；行为规格以 `openspec/specs/` 为准（`admin-portal-gate` change 实现门户准入）。
...
### 2.4 多租户模式（`relayflow.tenant.enabled=true`）

V2 在 §2.1–§2.3 语义不变的前提下，增加**企业（tenant）维度**：

| 项 | 行为 |
|----|------|
| **JWT `tenant_id`** | 表示用户**当前选中的企业**；切换企业后重签 JWT |
| **一账号多企业** | 同一 `sys_user` 可有多条 `sys_tenant_user`（ACTIVE）；登录时若未指定企业且有多企业，返回 `TENANT_SELECTION_REQUIRED` |
| **开放注册** | `allow-open-register=true` 时，`POST /app-api/system/auth/register` 创建 user + 新 tenant + owner；并激活该手机号全部 `NOT_JOINED` 邀请 |
| **管理端邀请** | `POST /admin-api/system/user/invite` 的 tenant **必须**来自管理员 JWT 的 `tenant_id`（禁止静默 fallback 到 default-id） |
| **`isAdmin`** | **按当前 JWT tenant** 判定：在该企业下是否绑定带 permission 的管理角色 |
| **员工工作台** | Rail 展示当前企业名；多企业时出现切换器（`POST /app-api/system/tenant/switch`） |

`enabled=false`（V1 自托管）时：无切换器、无开放注册；全员隐式绑定 `default-id`（通常 `1`）。

配置与环境变量见 [`README.md`](../../README.md) §租户产品模式、`deploy/.env.example`。
...
`admin-portal-gate` change 已实现以下行为；archive 后本节可删或改为「已落地」摘要。

| 项 | 目标态 | 实现 |
|----|--------|------|
| `/app/**` 未登录 | 跳转 login | ✅ 路由守卫 |
| `/admin/**` 未登录 | 跳转 login | ✅ 路由守卫 |
| `/admin/**` 已登录非 admin | 无权限引导 | ✅ `/app/no-admin-access` |
| 工作台「管理后台」入口 | 仅 `isAdmin` 可见 | ✅ `WorkspaceRail` |
| `/admin-api/**` 零 permission 用户 | 403（白名单除外） | ✅ `AdminPortalAuthorizationFilter` |
...
| `openspec/specs/web-auth/spec.md` | 登录门禁、未认证重定向 |
| `openspec/specs/web-admin/spec.md` | 管理端门户准入、`isAdmin`、无权限引导页 |
| `openspec/specs/system/spec.md` | RBAC 运行时、`isAdmin` 判定、管理面 API 门户准入 |
