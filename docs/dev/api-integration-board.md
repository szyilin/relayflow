...
| **workspace-task-group-by-field** | **archived** | **ui_ready** | `PUT …/group-move` · `/app/tasks` 按字段分区/看板列 | [contract](../../openspec/lanes/workspace-task-group-by-field/contract.md) | `-api` 完成；`-web` Mock 待 integrate；`board-move` 过渡兼容 |

**已交付顺序**：`list-web` → `list-api` → `list-integrate` → `board-web` → `board-api` → `board-integrate`。

**下一母 change 建议顺序**：`quick-views` ✅ → `view-config` ✅ → `group-by-field`（web ✅ / api ✅ → integrate）→ `multi-assignee` → `assigner`（部分已随 quick-views）→ `mine-groups` → `multi-list` → `list-groups` →（可选）`custom-field`。
