...
| **workspace-task-view-config** | **planned** | **ui_ready** | ViewConfig 工具栏 · `/app/tasks` | [contract](../../openspec/lanes/workspace-task-view-config/contract.md) | `-web`；本地 localStorage 暂存；待 `-api` |

**已交付顺序**：`list-web` → `list-api` → `list-integrate` → `board-web` → `board-api` → `board-integrate`。

**下一母 change 建议顺序**：`quick-views` ✅ → `view-config`（web ✅ → api）→ `group-by-field` → `multi-assignee` → `assigner`（部分已随 quick-views）→ `mine-groups` → `multi-list` → `list-groups` →（可选）`custom-field`。
