...
| **workspace-task-view-config** | **archived** | **ui_ready** | `GET/PUT …/view-config/*` · `/app/tasks` 工具栏 | [contract](../../openspec/lanes/workspace-task-view-config/contract.md) | `-api` 完成；待 `-integrate` 关 `USE_LOCAL_VIEW_CONFIG` |

**已交付顺序**：`list-web` → `list-api` → `list-integrate` → `board-web` → `board-api` → `board-integrate`。

**下一母 change 建议顺序**：`quick-views` ✅ → `view-config`（web ✅ → api ✅ → integrate）→ `group-by-field` → `multi-assignee` → `assigner`（部分已随 quick-views）→ `mine-groups` → `multi-list` → `list-groups` →（可选）`custom-field`。
