...
- 列类型使用 `JSONB`（如 `infra_notify.payload_json`）。
...
- **Flyway 脚本是表结构唯一真源**；Java DO 与基础 Mapper 须通过 MyBatis-Plus Generator 连库生成，输出至 `target/generated-sources/`，不提交 Git。
- 禁止 AI 或人工在 `src/` 下按表结构手写 `*DO` / 基础 `*Mapper`；自定义 SQL 使用 `{Entity}ExtMapper` + XML。
