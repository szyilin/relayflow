...
`{module}` 与 Maven 域模块对应：`system`、`infra`、`im`、`task`、`calendar`、`bpm`（V1.1）。

**鉴权语义**（产品面 vs 管理面、401/403 含义）见 [`product-permission-model.md`](product-permission-model.md)。摘要：`/app-api` 要求有效组织成员；`/admin-api` 要求管理身份及各端点 `permission.code`。
...
**前端义务**：调用方 MUST 根据 `total` 提供分页或「加载更多」；**禁止**固定请求 `pageSize: 100`（或任意单页）且无翻页 UI，导致用户无法看到后续数据。服务端上限 100 不是「一次拉全量」的许可。
