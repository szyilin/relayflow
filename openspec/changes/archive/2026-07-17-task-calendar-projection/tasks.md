...
- [x] 1.7 浏览器：`/app/calendar` 开关图层见/隐任务；点击进任务页；设置窗改默认
...
- [x] 2.1 `GET /app-api/task/item/due-range`（from/to、TODO、本人、租户、limit≤200）
- [x] 2.2 preference 代码默认合并 `settings.calendar.showTaskLayer=true`
- [x] 2.3 （可选）`TaskItemApi` due-range 同语义；日历页不依赖服务端聚合
- [x] 2.4 `./mvnw -pl relayflow-server -am compile` + curl 自测 due-range
...
- [x] 3.1 去掉 calendar/task store 内 due-range 临时数据；只走真实 API
- [x] 3.2 联调：有/无截止日、DONE 不出现、图层关不挡日程、深链 `taskId`
- [x] 3.3 `pnpm build && pnpm typecheck`；`./mvnw -pl relayflow-server -am compile`
- [x] 3.4 看板 → archived；子 change archive；母 change 勾选完成项
