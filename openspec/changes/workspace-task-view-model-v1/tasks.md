...
- [ ] 2.1 开 `workspace-task-quick-views-web`：左栏全部/我创建的/我分配的/已完成 + 默认筛选种子 Mock；contract
...
- [ ] 4.1 开 `workspace-task-group-by-field-web`：列表分区 + 看板列由字段 `groupBy` 驱动；「无分组」；拖拽 Mock
- [ ] 4.2 开 `workspace-task-group-by-field-api`：分组查询与拖拽落库；废弃说明旧 `board-move`
...
- [ ] 8.1 开 `workspace-task-multi-list-web`：详情多清单加入/移出
