...
模块依赖：`server → *-biz`；`*-biz → 其他 *-api`；**禁止** `*-biz → *-biz`。跨域同步走 `*-api`，异步走领域消息（见 [architecture.md](docs/dev/architecture.md)、[cross-domain-messaging.md](docs/dev/cross-domain-messaging.md)）。
...
**下一优先**：① 群 Bot / 可交互卡片（foundation §7.4–§7.7 开子 change）→ ② **`bpm-v1`** 主体实现（触达规格已修订为 `approval-bot`）→ ③ `im-bot-notify-foundation` 母 change archive（§7 子 change 开完后）。产方迁移：`im-bot-invite-migrate` / `im-bot-task-due-migrate` / `im-bot-reach-policy-v1` 已完成；`workspace-search-v1`、`notify-inbox-v2` 已 archive。
...
| 跨域调用 | 同步 `*-api` + 异步领域消息；禁止 `*-biz → *-biz` |
| 运行时 V1 | 仅 `relayflow-server`（无 Gateway/Nacos） |
| 前端 | 仅 `web/`，Nuxt UI v4，禁止 React |
| DO / 基础 Mapper / Mapper.xml | Flyway 真源 + `./scripts/codegen.sh --tables …` → 临时目录 → **diff 合入 `*-biz/src/`（进 Git）**；自定义 SQL 仅 ExtMapper |
