...
**下一优先**：按产品缺口立项 `org-member-invite-notify`（邀请通知）、`workspace-tasks-*`（工作台任务）、`im-channel-*`（频道）。`im-deepening-v1`（群聊 / 附件 / 已读 / 在线状态）、`multi-tenant-account-v2`、`workspace-profile-card` 已归档。
