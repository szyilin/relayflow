...
**下一优先**：`multi-tenant-account-v2` 收尾（§9 deployment 文档 → §10 归档同步 specs）；其后按产品缺口立项 `org-member-invite-notify`（邀请通知）、`workspace-tasks-*`（工作台任务）。V2 多租户账号（注册 / 切换 / 邀请迁移）§1–§8 已完成。
